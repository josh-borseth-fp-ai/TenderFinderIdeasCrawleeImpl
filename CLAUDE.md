...
Streaming chat and source onboarding: Bun + TypeScript 7 + TanStack Start (React 19) + Effect v4 + Effect Atom + shadcn/ui, OpenRouter as the model provider. Conversations, sources, packages, confirmations, jobs, collection snapshots, and opportunity records persist in SQLite.
...
- Source onboarding lives in the main chat; `/sources` redirects to a saved conversation. It uses the shared Effect HttpApi at `/api/source-data/*`, `/api/sources` for artifact downloads, and `/api/source-events` for replayable SSE. `src/atoms/sources.ts` uses AtomHttpApi for query/mutation state; conversation selection belongs to TanStack Router. `ConversationService` routes URL discovery and conversational intent; `ConversationIntentModel` returns schema-validated decisions. Collections page records from SQLite, and source actions are idempotent. `src/server/onboarding` uses Drizzle SQLite with migrations in `drizzle/`, Effect Queue/fibers for jobs, structured Effect AI decisions, and package validation. Use `bun run db:generate` after editing `db/schema.ts`. Browser disconnects do not cancel source jobs; explicit cancellation does.
- Scraper package assembly validates `PackageDraft` with Effect Schema and renders `runner/SCRAPER.md.hbs` with Handlebars (strict Markdown output). Runtime versions come from `runner/package.json`; internal validation/manual limits and the stored validation sample cap live in `runner/contract.ts`. Keep docs and execution on those shared values. Handlebars handles document templating, a capability outside Effect's scope.
...
- Resumable collection uses runner contract v2, persistent Crawlee queues and browser cursors in a per-collection directory. The worker persists and streams record chunks before advancing requests; the app replays interrupted delivery into SQLite and stores records individually. Internal validation precedes full collection, and user confirmation follows it. Legacy packages retain contract v1 and their pinned image.

- `POST /api/chat` (`src/routes/api/chat.ts`) validates `SavedChatRequest` (legacy `ChatRequest` remains supported), persists turns and runs `ConversationService`/`ChatService.stream`, and returns SSE via `src/server/Sse.ts`. Provider failures become a terminal `{"_tag":"Error"}` event, never a mid-stream 500. `GET /api/chat` returns `{ model }`.
- `src/server/ChatService.ts#partToEvents` maps provider stream parts to wire events. `ScrapeToolkit` provides `scrapeUrl`; request-local history carries tool results into one final model call with tool execution disabled. Tool calls/results remain server-side; SSE uses TextDelta/Error/Done plus Message references for saved turns and sources.
...
- Client: `ChatClient` streams saved turns through Effect HttpClient; `sendMessageAtom` merges optimistic messages with persisted conversation data. AtomHttpApi owns conversations, source actions, and paginated results. Source SSE and scoped reconciliation restore background updates. Stop interrupts the current text response; a source card's Stop explicitly cancels its job. Retry reuses the turn ID. New chat preserves previous conversations.
