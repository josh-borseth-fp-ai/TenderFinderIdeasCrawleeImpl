...
import type { AgentDecision, Evidence, SourceBrief } from "../src/domain/Source.ts"
...
globalThis.__onboardingService = new OnboardingService(new SourceStore(data), {
  ...
  const context = JSON.parse(prompt) as { brief: SourceBrief; evidence: Evidence[] }
...
test('extracts title and buyer from the observed detail', () => { const fixture = fixtures.find((page: {finalUrl: string}) => page.finalUrl.endsWith('/detail')); const rows = extract({...fixture, url: fixture.finalUrl, label: 'detail'}); assert.equal(rows[0]?.title, 'Bridge repair'); assert.equal(rows[0]?.buyer, 'City Works'); });
test('discovers the detail from rendered index', () => { const fixture = fixtures.find((page: {finalUrl: string}) => !page.finalUrl.endsWith('/detail')); assert.ok(discover({...fixture, url: fixture.finalUrl, label: 'index'}).some(row => row.url === 'http://fixture.test/detail')); });`,
  } }
})
